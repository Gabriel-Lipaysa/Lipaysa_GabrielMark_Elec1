...
<div class="container shadow my-3 bg-body rounded mx-3 p-4">
    <form action="{{ route('students.grades.update', ['studentId' => $studentId, 'gradeId' => $grade->id]) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" value="{{ $grade->course_name }} ({{ $grade->course_code }})" readonly>
        </div>

        <div class="form-group mt-3">
            <label for="grade">Grade</label>
            <input type="number" name="grade" class="form-control" value="{{ $grade->grade }}" step="0.5" min="1" max="5" required>
        </div>
